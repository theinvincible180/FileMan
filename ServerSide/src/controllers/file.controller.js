...
const uploadFiles = async (req, res) => {
  ...
  try {
    ...
    for (const file of req.files) {
      ...
      const finalFileName = `${originalName.replace(
        /\s+/g,
        "_"
    ...
    return res.status(500).json({ error: "File uploading failed!" });
  }
};
...
const downloadInfo = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      downloadContent: file.downloadedContent,
...
const updateFileStatus = async (req, res) => {
  ...
  try {
    ...
    if (file.status === status) {
      return res.status(400).json({ error: "File already has this state" });
    }
...
const updateFileExpiry = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: "Failed to update file expiry" });
  }
};
...
const updateAllFileExpiry = async (req, res) => {
  ...
  try {
    ...
    for (const file of files) {
      if (files.status === "deleted") continue;
      if (file?.expiresAt && new Date(file.expiresAt) < new Date()) {
        file.status = "expired";
        file.hasExpiry = true;
      } else {
    ...
    return res
      .status(200)
      .json({ message: "All file expiries updated successfully" });
  } catch (error) {
...
const updateFilePassword = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: "Failed to update file password" });
  }
};
...
const showUserFiles = async (req, res) => {
  ...
  try {
    ...
    console.error("Error showing user files:", error);
    return res.status(500).json({ error: "Failed to show user files" });
  }
};
...
const generateShareShortenLink = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: "Failed to generate short link" });
  }
};
...
const sendLinkEmail = async (req, res) => {
  ...
  try {
    ...
    const mailOptions = {
      from: `"File Man App" <${process.env.MAIL_USER}>`,
      to: email,
    ...
    <div style="font-family: Arial, sans-serif; line-height: 1.6;">
      ...
      <p>Hello,</p>
      <p>You have been sent a file using <strong>File Man App</strong>.</p>
      ...
      <p>Thank you for using File Man App!</p>
    </div>
  `,
    };
...
const getDownloadCount = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({downloadCount: file.downloadedContent})
  } catch (error) {
